...
bool SquareTest::Initialize(ID3D11Device* device)
{
	bool result;
	hasTexture = false;
...
void SquareTest::Shutdown()
{
	ShutdownBuffers();
	return;
}

void SquareTest::Render(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatix, 
...
int SquareTest::GetIndexCount()
{
	return m_indexCount;
}

bool SquareTest::InitializeBuffers(ID3D11Device* device)
{
	VertexType* vertices;
...
void SquareTest::ShutdownBuffers()
{
...
void SquareTest::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
...
bool SquareTest::LoadTexture(ID3D11Device*)
{
	return true;
}

void  SquareTest::ReleaseTexture()
{
