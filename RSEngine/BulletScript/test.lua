...
function updateFrame()
	...
	local tmpt = 1
		for i=1, 90 do 
			...
			elseif(i <= 70) then
			    vetx[tmpt] = 400 - currentFrame + (i-50) * 50
			elseif(i <= 90) then
			    vetx[tmpt] = 400 - currentFrame + (i-70) * 50
            end				
			tmpt = tmpt + 1
			if (i <= 50) then
				vetx[tmpt] = 300 - i * 50
			...
			if (i <= 50) then
				vetx[tmpt] = 100
			elseif(i <= 70) then
				vetx[tmpt] = 300 - (i - 50) * 30
			elseif(i <= 90) then
				vetx[tmpt] = 300 - (i - 70) * 30
			end	
			tmpt = tmpt + 1
		end
end
...
function updateInterpolation(intp)
	...
	local tmpt = 1
		for i=1, 90 do 
			...
			elseif(i <= 70) then
			    vetx[tmpt] = 400 - currentFrame + (i-50) * 10 - intp
			elseif(i <= 90) then
			    vetx[tmpt] = 400 - currentFrame + (i-70) * 10 - intp
            end				
			tmpt = tmpt + 1
			if (i <= 50) then
				vetx[tmpt] = 300 - i * 50
